...
const visitor = new Visitor("9E1005A551ED61CA0A490D45@AdobeOrg");
...
test("Visitor Interface:", (t) => {
    ...
    t.ok(visitor.getCookieName, "getCookieName");
    t.end();
});

test("calling getCookieName", (t) => {
    t.equals(visitor.getCookieName(), "AMCV_9E1005A551ED61CA0A490D45@AdobeOrg", "should return AMCV_ + orgID");
    t.end();
});

test("generatePayload should return an SDID only if no cookie exist", (t) => {
    const visitorPayload = visitor.generatePayload({ consumerID });
...
test("Calling getState:", (t) => {
    ...
    t.ok(state["9E1005A551ED61CA0A490D45@AdobeOrg"], "State should have be partitioned by OrgID");
    t.ok(state["9E1005A551ED61CA0A490D45@AdobeOrg"].sdid, "State should contain a SDID");
    t.equal(state["9E1005A551ED61CA0A490D45@AdobeOrg"].consumerID, "payload:test-customer-A");
    t.end();
});
...
test("Calling Visitor without an orgID", (t) => {
    ...
    t.throws(invalidInstantiation, Error, "Should throws an Org ID required exception");
    t.end();
});

test("Calling Visitor without an invalid orgID", (t) => {
    const invalidIDs = {
        shortAndNoAdobeOrgID: "2312125",
        shortOrgId: "2312125@AdobeOrg",
        wrongCaseOrgID: "1234567891011@adobeorg",
        exactDigitsNoAdobeOrgID: "1234567891011"
    };

    Object.keys(invalidIDs).forEach((k) => {
        const invalidInstantiation = () => {
            new Visitor(invalidIDs[k]);
        };

        t.throws(invalidInstantiation, Error, "Should throws an invalid Org ID exception");
    });
